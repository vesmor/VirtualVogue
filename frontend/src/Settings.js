...
const Settings = () => {
  ...
  const deleteAccount = async () => {
    const check = window.confirm(
      "Are you sure you want to delete your account? This data cannot be recovered later."
    );
    if (!check) return;
    ...
    try {
      ...
      if (userData) {
        ...
        const response = await fetch(
          buildPath(`api/DeleteUser/${parsedUserData.userId}`),
          {
            method: "DELETE",
          }
        );
  ...
  const saveChanges = async (event) => {
    ...
    if (!emailRegex.test(email)) {
      ...
      error =
        "Password must be 8 characters long, one upper and lower case letter, a number, and a special symbol. Changes were not applied.";
      isValid = false;
    }
    ...
    try {
      const response = await fetch(buildPath("api/UpdateSettings"), {
        method: "POST",
        body: js,
